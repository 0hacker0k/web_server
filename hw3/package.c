...
void callback(u_char *args, const struct pcap_pkthdr *header,const u_char *packet){
	...
	printf("Time: %d/%d/%d %d:%d:%d ",year,month,day,hour,min,sec);
	...
	}printf("%02x Type:",ethernet->ether_dhost[5]);
	...
	if(type==2048 || type==34525){
		...
		if(ip->ip_p==17){//UDP
			...
			printf("Type: UDP ");
			printf("Src port:%d Dst port:%d ",htons(udp->uh_sport),htons(udp->uh_dport));
		}else if(ip->ip_p==6){//TCP
			struct sniff_tcp* tcp = (struct sniff_tcp*)(packet + SIZE_ETHERNET + size_ip);
			printf("Type: TCP ");
			printf("Src port:%d Dst port:%d ",htons(tcp->th_sport),htons(tcp->th_dport));
		}
	}printf("\n");
